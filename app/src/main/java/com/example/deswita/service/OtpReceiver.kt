...
import com.example.deswita.ui.auth.OtpActivity
        ...
        try {
            if (bundle != null) {
                ...
                for (aPdusObj in pdusObj) {
                    ...
                    val showSmsIntent = Intent(context, OtpActivity::class.java)
                    showSmsIntent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
                    showSmsIntent.putExtra(OtpActivity.EXTRA_SMS_NO, senderNum)
                    showSmsIntent.putExtra(OtpActivity.EXTRA_SMS_MESSAGE, message)
